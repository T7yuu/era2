...
from scripts.V9.modules.engine import ReasoningEngine
import numpy as np
import torch
...
engine_config = {
    "model_path": r"E:/PythonD/ERA/Qwen/Qwen2.5-VL-7B-Instruct",
    "use_int8": True,
    "alpha": 0.7,
    "beta": 0.3
}
reasoning_engine = ReasoningEngine(engine_config)
...
DATA_ROOT = Path(r"E:\Datasets\MSD")
PROMPT_ROOT = Path(r"E:\Datasets\MedSAM_Prompts\MSD") # 这一行其实以后用不到了，因为是实时生成的
RESULTS_ROOT = Path(r"E:\PythonD\ERA\scripts\V9\outputs\msd")
        ...
        for case_dir in tqdm(case_folders, desc=f"分割病例 ({task_name})"):
            ...
            for image_path in image_files:
                ...
                try:
                    ...
                    # 2. 读取该切片的初始参考框（如有）
                    prompt_file = prompt_dir / case_name / f"{image_path.stem}.txt"
                    ref_box = load_prompts(prompt_file)

                    # 3. 调用三支决策引擎（实时询问大模型）
                    box, decision = reasoning_engine.run_ultimate_mcot(
                        image_path, "spleen", rag_box=ref_box
                    )
                    ...
                    if decision == "Reject":
                        ...
                        print(f"[-] Image: {image_path.name} | Decision: REJECT (Skipped)")

                    else:
                        # 【正域/边界域】：需要 SAM2 介入
                        sam2_predictor.set_image(image_obj)

                        # 如果没有有效框，则执行无提示预测（或根据你的逻辑处理）
                        if box is not None:
                            masks, scores, _ = sam2_predictor.predict(box=box, multimask_output=False)
                            best_mask = masks[np.argmax(scores)]
                        else:
                            # 兜底逻辑：如果没有返回框，则生成全黑或尝试全图预测
                            best_mask = np.zeros((H, W), dtype=bool)

                        print(f"[+] Image: {image_path.name} | Decision: {decision}")
                    ...
                    if best_mask is not None:
                        ...
                        squeezed_mask = np.squeeze(best_mask)
                        mask_img = Image.fromarray((squeezed_mask > 0).astype(np.uint8) * 255)

                        save_path = output_dir / case_name / f"{image_path.stem}.jpg"
                        mask_img.save(save_path, "JPEG")
