...
def load_prompts_3wd(prompt_file: Path):
    """
    修改后的提示加载函数，支持三支决策逻辑
    返回: (boxes, decision_type)
    """
    if not prompt_file.exists():
        return None, "Reject"  # 找不到文件视为拒绝

    with open(prompt_file, 'r', encoding='utf-8') as f:
        content = f.read().strip()

    if content.lower() == "null":
        return None, "Reject"  # 负域：明确无目标

    if "uncertain" in content.lower():
        # 这里假设你在生成提示时，如果不确定会写入 uncertain 和坐标
        decision = "Defer"
    else:
        decision = "Accept"
    ...
    lines = content.split('\n')
    for line in lines:
        if "uncertain" in line.lower(): continue
        try:
            coords = [float(c) for c in line.strip().split()]
            if len(coords) == 4:
                boxes.append(coords)
    ...
    return np.array(boxes), decision
    ...
    for image_path in tqdm(image_files, desc="正在分割图片"):
        try:
            ...
            with torch.inference_mode(), torch.autocast(DEVICE, dtype=torch.bfloat16):
                ...
                prompt_file = PROMPT_DIR / f"{image_path.stem}_segmentation.txt"

                # --- 调用修改后的三支决策加载逻辑 ---
                box_prompts, decision = load_prompts_3wd(prompt_file)
                ...
                if decision == "Reject":
                    ...
                    logging.info(f"图片 {image_path.name} 判定为[负域]，跳过分割。")

                elif decision == "Accept" or decision == "Defer":
                    # 【正域】或【边界域】
                    # 如果是 Defer，你也可以选择在这里增加逻辑（比如膨胀框或者取多框并集）
                    # 目前这里统一使用 SAM2 进行预测
                    masks, scores, _ = sam2_predictor.predict(
                        box=box_prompts,
                        multimask_output=False,
                    )
                    # 即使模型给了框，如果 SAM2 预测分数极低，也可以二次判定为 Reject（三支决策的进一步应用）
